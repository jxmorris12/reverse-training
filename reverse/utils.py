...
def get_model_params(model: torch.nn.Module) -> Tensor:
    return torch.cat([p.data.reshape(-1) for p in model.parameters()], 0) # .requires_grad_(True)

def get_model_grad(model: torch.nn.Module) -> Tensor:
    try:
        return torch.cat([p.grad.data.reshape(-1) for p in model.parameters()], 0)
    except AttributeError:
        # 'NoneType' object has no attribute 'data'
        # -> grad is zero / None
        return torch.zeros_like(get_model_params(model))